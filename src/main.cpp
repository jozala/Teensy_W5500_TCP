...
void loop() {
  ...
  if (now - lastTcpPublishTime > TCP_PUBLISH_INTERVAL_MS) {
    ...
    ethClient.write("2. This is another message to send more and check if it influence stability\n", 76);
    ethClient.write("3. One more check if it influence stability\n", 44);
    ethClient.write("4. End even more more and check if it influence stability\n", 58);
